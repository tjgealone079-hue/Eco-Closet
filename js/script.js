    ...
    function showPage(id) {
      ...
      const isAuth = authPages.includes(id);

      homeBtn.forEach(el => el.style.display = isAuth ? "" : "none");
      loginBtn.forEach(el => el.style.display = isAuth ? "" : "none");
      logoutBtns.forEach(el => el.style.display = isAuth ? "none" : "");
    }

    addClickAll(brandBtn, () => showPage("landingPage"));
    addClickAll(homeBtn, () => showPage("landingPage"));
    addClickAll(loginBtn, () => showPage("loginPage"));
    ...
    addClickAll(itemsBtn, () => requireLogin(() => showPage("itemsPage")));
    addClickAll(cartBtn, () => requireLogin(() => { updateCart(); showPage("cartPage"); }));
    addClickAll(paymentBtn, () => requireLogin(() => showPage("paymentPage")));
    addClickAll(checkOutBtn, () => requireLogin(() => showPage("checkoutPage")));

    function closeMobileNav() {
      document.querySelector(".nav-mobile .nav-links").style.display = "none";
    }

    document.getElementById("navMobileToggle").addEventListener("click", () => {
      const links = document.querySelector(".nav-mobile .nav-links");
